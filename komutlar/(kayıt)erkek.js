...
if(!message.member.roles.cache.has(kayıtçı)) return message.channel.send(`⛔ Bu Komudu Kullanabilmen İçin <@&${kayıtçı}> Rolüne Sahip olman Lazım ! `)
...
let isim = args[1]
if (!isim) return message.channel.send(`⛔ Kullanıcının İsmini Belirtmelisin ! `)
let yaş = args[2]
if (!yaş) return message.channel.send(`⛔ Kullanıcının Yaşını Belirtmelisin ! `)
